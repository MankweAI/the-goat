...
function AppContent() {
  ...
  const handlePlanHomework = async (formData) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error((await response.json()).error || "Failed to plan homework session.");
      
      const data = await response.json();
      if (!data || !data.plannedQuestions || data.plannedQuestions.length === 0) {
        setError("I couldn't find any questions on that page. Please try again.");
  ...
  const handleStartObjective = async (objective) => {
    ...
    const parentQuestion = plannedQuestions.find(q => q.curriculum === curriculum);
    ...
    try {
        ...
        if (!response.ok) throw new Error((await response.json()).error || "Failed to generate lesson.");
  ...
  const handleLessonComplete = async () => {
    ...
    try {
      if (currentObjective.type === 'homework') {
        ...
        if (!response.ok) throw new Error((await response.json()).error || "Failed to generate solution.");
  ...
  const handleObjectiveMastered = () => {
    const newCompletedObjectives = new Set(completedObjectives).add(currentObjective.id);
    setCompletedObjectives(newCompletedObjectives);

    const isCurrentCurriculumComplete = curriculum.every(obj => newCompletedObjectives.has(obj.id));

    if (isCurrentCurriculumComplete) {
      if (currentObjective.questionId) {
        const newCompletedHwIds = new Set(completedHomeworkIds).add(currentObjective.questionId);
        setCompletedHomeworkIds(newCompletedHwIds);
        
        if (plannedQuestions.length > 1 && newCompletedHwIds.size < plannedQuestions.length) {
          setScreen("homework_selection");
          return;
        }
      }
  ...
  const renderScreen = () => {
    if (error) { return ( <div className="w-full max-w-md mx-auto text-center p-4"> <p className="p-4 bg-red-100 text-red-700 rounded-lg">{error}</p> <button onClick={handleGoHome} className="mt-4 text-sm text-gray-600 hover:underline"> Go Home </button> </div> ); }
    if (isLoading || screen === 'loading') return <LoadingSpinner />;

    switch (screen) {
      case "home":
        return <HomeScreen onStartTopicMastery={() => setScreen("topic_intake")} onStartHomework={() => setScreen("homework_intake")} />;
      case "homework_intake":
        return <HomeworkScreen onGenerateCurriculum={handlePlanHomework} onBack={handleGoHome} isLoading={isLoading} />;
      case "homework_selection":
        return <HomeworkSelectionScreen questions={plannedQuestions} completedQuestionIds={completedHomeworkIds} onSelectQuestion={(question) => { setCurriculum(question.curriculum); setCompletedObjectives(new Set()); setScreen("topic_curriculum"); }} onBack={handleGoHome} />;
      case "topic_intake":
        return <TopicIntakeScreen onGenerateCurriculum={handleGenerateMasteryCurriculum} onBack={handleGoHome} isLoading={isLoading} />;
      case "topic_curriculum":
        return <CurriculumScreen curriculum={curriculum} onStartObjective={handleStartObjective} onBack={() => plannedQuestions.length > 1 ? setScreen('homework_selection') : handleGoHome()} completedObjectives={completedObjectives} />;
      case "lesson":
        return <LessonScreen lessonPlan={currentLessonPlan} objective={currentObjective} onBack={() => setScreen("topic_curriculum")} onLessonComplete={handleLessonComplete} />;
      case "solution":
        return <SolutionScreen solution={currentSolution} objective={currentObjective} onContinue={handleObjectiveMastered} />;
      case "topic_challenge":
        return <ChallengeScreen challenges={currentLessonPlan.challenges} onChallengesComplete={handleChallengesComplete} onBack={() => setScreen("topic_curriculum")} />;
      case "topic_progress_report":
        return <ProgressReportScreen results={challengeResults} onContinue={handleObjectiveMastered} />;
      // ... (mastery quiz case)
      default:
        return <HomeScreen onStartTopicMastery={() => setScreen("topic_intake")} onStartHomework={() => setScreen("homework_intake")} />;
    }
